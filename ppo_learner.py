...
if __name__ == "__main__":
    ...
    env = make_env(args, run_name, record_video=False)() # learner should not record video as it doesn't interact with environment
    ...
    print("[Learner] Initial weights sent to actors.")
    ...
    try:
        for iteration in range(1, args.num_iterations + 1):
            ...
            advantages, returns, learner_values = compute_advantages(
                batch, agent, args, device
            )

            current_logprobs = None
            if args.weighting_strategy == "is":
                with torch.no_grad():
                    _, current_logprobs, _, _ = agent.get_action_and_value(
