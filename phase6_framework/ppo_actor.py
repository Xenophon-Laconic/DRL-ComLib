...
if __name__ == "__main__":
    ...
    try:
        while not should_stop:
            ...
            # First consume any learner control message before flushing cache.
            ...
            if sync_status == "shutdown":
                ...
                should_stop = True
                break
            elif sync_status == "reset":
                print(f"[Actor {args.actor_id}] Reset applied from learner.")

            # Only now send the current batch / flush cached batches.
            comms.send_batch(batch, episode_stats)

            # Optional second sync: pick up fresh broadcasts promptly after sending.
            sync_status = comms.sync_weights(actor)
