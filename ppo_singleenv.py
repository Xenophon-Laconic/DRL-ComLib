...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # changed for single env !
            next_obs, reward, termination, truncation, info = env.step(action.item())
            next_obs = torch.tensor(next_obs, dtype=torch.float32).to(device)  # ← add this
            next_done_scalar = float(termination or truncation)
            rewards[step] = torch.tensor(reward, dtype=torch.float32).to(device)
            next_done = torch.tensor(next_done_scalar).to(device)
        ...
        # changed for single env
        b_obs      = obs          # already (num_steps, obs_dim)
        b_logprobs = logprobs     # already (num_steps,)
        b_actions  = actions      # already (num_steps,)
